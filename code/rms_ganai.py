...
def create_model_gen_train(inputs, targets):
	...
	with tf.variable_scope("generator", reuse=True) as scope:
		out_channels = int(targets.get_shape()[-1])
		outputs, latent_vector_real = create_generator(inputs, out_channels)
	...
	with tf.name_scope("generator_loss"):
		...
		gen_loss_GAN = tf.reduce_mean(-tf.log(gan_predict_fake + EPS))
		gen_loss_L1 = tf.reduce_mean(tf.abs(targets - outputs))
		gen_loss = gen_loss_GAN * a.gan_weight + gen_loss_L1 * a.l1_weight + tf.reduce_mean((latent_vector_real - latent_vector_fake)**2) 
